...
vel = "a"
while vel.isalpha():
    vel = input("Ingrese la rapidez del vehículo en kilómetros por hora: ")
...
ax.plot(x_valoresPol,y_valoresPol, "-k",linewidth = 10, label = "Curva Generada")
...
ax.plot(x_valoresPol,y_valoresPol, "--w", linewidth = 1.5)
...
carro = ax.plot([x_valoresPol[0], y_valoresPol[0]], label=f"Rapidez = {vel} m/s")
ax.set(xlim=[30, 260], ylim=[50, 400])
ax.legend()

def update(frame):
    carro.set_xdata(x_valoresPol[:frame])
    carro.set_ydata(y_valoresPol[:frame])
    return(carro)
